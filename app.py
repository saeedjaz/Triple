# app.py
# =========================================================
# منصة TriplePower - جدول الأهداف (Wide: يومي + أسبوعي)
# - "بداية الحركة (أسبوعي)" >= قمة آخر شمعة بيعية شهرية معتبرة (بوابة شهرية)
# - الأهداف الأسبوعية تُبنى على مدى الشمعة الأسبوعية فقط
...
# - اختيار "الشمعة البيعية المعتبرة" (بنفسها أو ما بعدها) وفق TriplePower
# =========================================================
...
# --- اختيار آخر شمعة بيعية 55% "بنفسها أو ما بعدها" ---
def last_considered_sell_index(df: pd.DataFrame, pct: float = 0.55):
    """
    تُعيد مؤشر آخر شمعة بيعية 55% التي كُسر بعدها (بنفسها أو ما بعدها) قاع آخر شمعة شرائية 55%.
    """
    ...
    # قاع آخر شمعة شرائية 55% قبل كل i
    ...
    # أدنى Low من i وحتى النهاية (للتحقق من "كسر ما بعدها")
    ...
    considered = (c < o) & (np.where(rng != 0, np.abs(c - o) / rng, 0.0) >= pct) & (rng != 0) \
                 & ~np.isnan(prev_win_low) & (fwd_min_low <= prev_win_low)

    idx = np.where(considered)[0]
    return int(idx[-1]) if len(idx) else None

def get_last_sell_info(df_tf: pd.DataFrame):
    """
    يعيد (H, L, R) لآخر شمعة بيعية معتبرة 55% "بنفسها أو ما بعدها" على نفس الفاصل.
    """
    ...
    if j is None:
        return None
    H = float(df_tf["High"].iat[j]); L = float(df_tf["Low"].iat[j]); R = H - L
...
def resample_weekly_from_daily(df_daily: pd.DataFrame, suffix: str) -> pd.DataFrame:
    """أسبوعي مؤكد من اليومي + إسقاط الأسبوع الجاري إذا لم يُغلق."""
...
# =============================
# حساب البداية والأهداف (يومي/أسبوعي)
# =============================
def compute_daily_targets(df_d_conf: pd.DataFrame):
    info = get_last_sell_info(df_d_conf)
    if info is None: return None
    H, L, R = info
    start = H
    return round(start,2), round(start+R,2), round(start+2*R,2), round(start+3*R,2)

def compute_weekly_targets_with_monthly_gate(df_w: pd.DataFrame, df_m: pd.DataFrame):
    """
    بداية الحركة (أسبوعي) = max(H_w, H_m) حيث:
      - (H_w, L_w, R_w) من آخر شمعة بيعية أسبوعية معتبرة
      - H_m من آخر شمعة بيعية شهرية معتبرة (إن وُجدت)
    الأهداف = Start + n*R_w (باستخدام مدى الشمعة الأسبوعية فقط)
    """
    w_info = get_last_sell_info(df_w)
    if w_info is None: return None
    H_w, L_w, R_w = w_info
    H_m = None
    if df_m is not None and not df_m.empty:
        m_info = get_last_sell_info(df_m)
        if m_info is not None:
            H_m = m_info[0]
    start = max(H_w, H_m) if H_m is not None else H_w
    return round(start,2), round(start+R_w,2), round(start+2*R_w,2), round(start+3*R_w,2)
        ...
        for i in range(0, total, batch_size):
            ...
            for code in chunk_syms:
                try:
                    ...
                    df_d_state = detect_breakout_with_state(df_d_conf.copy())
                    daily_first_break = bool(df_d_state["FirstBuySig"].iat[-1])
                    ...
                    # ---- (1) أهداف اليومي ----
                    tp_d = compute_daily_targets(df_d_conf)
                    if tp_d is not None:
                    ...
                    # ---- (2) أهداف الأسبوعي مع بوابة شهرية ----
                    tp_w = compute_weekly_targets_with_monthly_gate(df_w, df_m)
                    if tp_w is not None:
