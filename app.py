...
# تهيئة الصفحة + RTL
# =============================
st.set_page_config(page_title="🎯 جدول الأهداف (الأسبوعي + اليومي) | TriplePower", layout="wide")
st.markdown("""
<style>
...
# =============================
if st.button("🔎 إنشاء جدول الأهداف (الأسبوعي + اليومي)", key="run_weekly_btn"):
    if not symbols:
    ...
    with st.spinner("⏳ نجلب البيانات ونحسب الأهداف..."):
        ...
        if rows:
            ...
            st.subheader(f"🎯 جدول الأهداف (الأسبوعي + اليومي) — {market_name} — {day_str} — عدد الرموز: {len(df_final)} {filt_note}")
